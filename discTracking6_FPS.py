...
def resizeBlurHSV(frame):
    ...
    hsv = cv2.cvtColor(blurred, cv2.COLOR_BGR2HSV) # Keep RBG color space?
    return hsv, frame
...
while True:
   ...
   # while(True):
    ...
    if (counter > 30):
        # for i in range(0, len(timePos)-1):
            # timeAdded += timePos[i]
        fps = counterForFPS / timeAdded
        print("Number of frames: ", counter)
        print("timeAdded: ", timeAdded)
        print("FPS: ", fps)
        counterForFPS = 0
        timeAdded = 0
...
# Implement FPS
# Get frame
# Use RGB; Find blue channel
# Make everything else that isnt blue black, blue white; maximum contrast, threshold the blue color
# remove counter, redo centroid

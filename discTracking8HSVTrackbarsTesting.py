...
while True:
    ...
    uHue = cv2.getTrackbarPos("U - H", "Trackbars")
    uSaturation = cv2.getTrackbarPos("U - S", "Trackbars")
    uValue = cv2.getTrackbarPos("U - V", "Trackbars")
    lHue = cv2.getTrackbarPos("L - H", "Trackbars")
    lSaturation = cv2.getTrackbarPos("L - S", "Trackbars")
    lValue = cv2.getTrackbarPos("L - V", "Trackbars")

    blueLower = (lHue,lSaturation,lValue)
    blueUpper = (uHue,uSaturation,uValue)
    ...
    if (len(pts) > 2):
        ...
        if ( lowerGoalBoundX < oldX < upperGoalBoundX ):   # Determine if x is within goal

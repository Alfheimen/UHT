...
def calculateDisplacement(counter):
    disX = pts[counter-1][0] - pts[counter][0]
    disY = pts[counter-1][1] - pts[counter][1]
    if np.abs(disX) or np.abs(disY) > 20:
        totalDisp = np.sqrt(np.square(disX) + np.square(disY))
        return totalDisp

def calculateVelocity(totalDisp, timeList, counter):
    # V = S / T

    velocity = totalDisp / (timeList[counter] - timeList[counter-1]) 
...
while True:
    ...
    start = time.time()         # Start and end time for velocity calculation
    frame = vs.read()
    end = time.time()
    timeList.append(end-start)
    ...
    print("Time for frame ", counter, ": ", timeList[counter])


#--------------Velocity calculation--------------#
    
    if(counter >= 1 and len(pts) >= 2):
            totalDisp = calculateDisplacement(counter)
            if (np.abs(totalDisp) >= 20):
                calculateVelocity(totalDisp, timeList, counter)

...
x           = 0
oldX        = 0
lowerGoalBoundX = (113, 0)
upperGoalBoundX = (227, 0)
...
def calculateTrajectory():
    deltaX = (pts[0][1]-82) - (pts[1][1]-82)
    deltaY = pts[0][0] - pts[1][0]
    
    if(deltaX != 0):                            # Ensure no division by zero
        m = deltaY / deltaX                     # Find slope
        if(m != 0):                             # Ensure no division by zero
            x = ((m*(pts[1][1]-82)) - (pts[1][0])) / m     # Calculate X-coordinate of intersection where Y = 0
            print("x: ",x)
            if(abs(x) > 5):
                return x
...
while True:
    ...
    if (len(pts) > 2):
        ...
        if ( lowerGoalBoundX < (oldX, 0) < upperGoalBoundX ):   # Determine if x is within goal boundaries
